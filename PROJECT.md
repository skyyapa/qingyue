...
**迭代 57 —— 检测把普通词误判成人物（实体类型分类修正）**
- 修复用户反馈：知识库分析后“人物”分类混进普通词/连接短语（如「和大人」「给自己」这类`介词+名词`场景被投了人名票）
- `voteContext`：`介词+个体词` 的弱人名证据（旧 `next!=='是'`）收窄为**个体词后紧跟说话动词**才投人心票（`对X说/跟X说`），不再在 `和X一起/给X自己` 等连接短语上 +2；个体词后接介词的人名票收窄为**与格介词** `对/向/跟`（X对Y/X向Y/X跟Y），排除 `和/与/同/给`（常连接非人物）
- `decideType`：人物票需**严格高于**其余类型才判人物，与具体类型（地点/技能/物品/势力/境界）平票时判具体类型而非人物
- 单测 +3（234：连接短语不误判、对X说仍判人物、平票判具体类型）；其余不变，e2e 59 全绿
