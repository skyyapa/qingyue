...
**迭代 57 —— 检测误判修正：名字不漏检 + 普通词不误判（实体类型分类收敛）**
- 修复用户反馈分两版收敛：第一版（“把普通词误判成人物”）过度收紧——`decideType` 要求人物票**严格高于**其余类型、`voteContext` 把介词人名票全部收窄为“个体词后紧跟说话动词”——导致真实名字漏检、普通词误报不减反增
- 定稿版**还原 `decideType` 先到先得仲裁**（≤2 判 unknown，姓名不因平票被判成具体类型），并保留两类宽泛人名信号：① **名字后接任意介词**（X对Y/X说，施动方多为人名）；② **强目标介词 `对/向/跟` 后的个体词后接任意非「是」**（保住 `对X拱手/向X点头` 等真名字场景，避免漏检）
- 仅对**弱连接/被动介词 `和/与/同/给/让/被/随/陪`** 收紧：个体词须后紧跟说话动词才投人物票（`和X说`），杜绝 `和大人/给自己/同城门` 等连接短语误判
- 回归：单测 +5（236：名字多样语境全判人物 `萧炎/云韵/林动`、常用名词 `大人/自己/一起/门前` 不判人物、`对X拱手/向X点头` 仍判人名、弱介词需说话动词才投、`对X说` 仍投人物）；e2e 59 全绿
