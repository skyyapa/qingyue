...
- **阅读助手（本地知识库）** —
  - 无 AI 成本：基于上下文规则的实体识别（如「X说」→ 人物、「在X」→ 地点、「使出X」→ 技能），建立全书章节索引；规则识别**可能误判**，所有实体都支持人工修正
  - 人物关系图（段落共现加权）、章节摘要（登场人物 / 地点 / **事件句**，如「林风对苏瑶说」）、前情回顾（时间线可点击跳章）
...
npm run test       # 单元测试（Vitest，143 用例：编码检测/EPUB 与 CSS 解析/知识库算法/书源引擎/AI 客户端与任务/数据库/Store/组件/备份/导出/搜索）
npm run e2e        # 端到端测试（Playwright 27 用例，首次需 npx playwright install chromium）
```
